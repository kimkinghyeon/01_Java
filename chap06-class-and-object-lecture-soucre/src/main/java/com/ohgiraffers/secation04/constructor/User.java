...
public class User {
    ...
    * 3. 작성한 생성자 외에는 인스턴스를 생성하는 방법을 제공하지 않곘다는 의미
    ...
    * 2. 매개변수 있는 생성자
    *
    * 매개변수가 있는 생성자가 한개라도 존재하면 기본생성자를 자바 compiler 가 자동으로 추가하지 않는다.
    * 기본 생성자가 필요한 경우에는 반드시 명시적을 작성을 해야한다. */
    ...
    public String getInfo(){
        return "User [id =" + this.id + " , pew = " + this.pwd + ", " +
                "name = " + this.name + ", enrollDate = " + this.enrollDate + "]";
    }

    // 매개변수 있는 생성자 (id, pwd, name의 초기화 담당)
    public User(String id,String pwd,String name){
        // 매개변수 잇는 생성자의 주목적 -> 인스턴스 생성 시점에 필드를 원하는 값으로 초기화하기 위함
        this.id = id;
        this.pwd =pwd;
        this.name =name;

        System.out.println("User 클래스의 id, pwd, name 를 초기화 하는 생성자 호출...");
...
//        this.enrollDate = enrollDate;
        ...
        * 괄호 안에 매개변수의 타입, 갯수, 갯수, 순서에 맞는 생성자를 호출하고 보귀한다.(메소드 통일)
        * this() 는 가장 첫 줄에 선성해야 하며, 그렇지 않는 경우 compile error 가 발생한다.
        * */
     ...
     *
     * -> 같은내용의 인스턴스를 만들떄 사용
     * 동일한 값을 가지지만 새롭게 할당되는 인스턴스이기떄문에 서로다른 주소값을 가지게 된다.
     * */
    public User(User otherUser){
        ...
        System.out.println("this 의 해시코드 : "+this.hashCode());
        System.out.println("otherUser 의 해시코드 : "+otherUser.hashCode());
}
    }
