...
public class Application {
    public static void main(String[] args) {
        ...
        * 갹체의 재사용성이 증가하면 중복 되는 코드를 줄일 수 있고, 오류 발생을 감소시킬 수 있다.
        *
        *
        * 객체와 객체의 상호작용
        * 객체와 객체는 메세지를 통해 서로 상호작용을 한다.
        * 이떄 메세지(메소드)를 보내는 쪽은 송신자,메시지(메소드)를 수신하는 쪽은 수신자
        * 수신자는 메세지를 전달 받아 그 메세지에 해당하는 내용을 처리하는 방법을
        * 스스로 결정하고, 그 결정할 방법 대로 처리할 명령어들을 순차적으로 기술한 것 ==> 메소드
        *
        * 상호작용에 중요한 것은 메세지를 송수신하고 그방법을 기술하여 메세지에 응답하는 상호작용을 설계하는것
        * => 따라서 객체를 설계할 떄는 속성은 부가적이며 적절한 책임을 설계하고
        ...
        * 1. 카레이서는 시동걸기, 엑셀레이터 밝기, 브레이크 밝기, 시동 끄기를 할 수 있다.
        * 2. 자동차는 시동걸기, 앞으로가기, 멈추기, 시동끄기를 할 수 있다.
        * 3. 자동차는 처음에 멈춘 상태로 대기
        * 4. 카레이서는 먼저 자동차에 시동을 건다.( 이미 걸려 있는 경우 다시 시동을 걸 수 없다.)
        * 5. 카레이서가 엑셀레이터를 밞으면 시동이 걸린 상태일 경우, 자동차는 시속이 10km/h 증가하며 앞으로 간다.
        * 6. 자동차가 달리는 중인 경우 브레이크를 밞으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밞을 떄 자동차가 달리는 중이 아니면 이미 멈춰있는 상태라고 안내한다.
        ...
        * 2). 가속하기
        *   - 사용자 --> 엑셀레이터를 밞아라 --> 카레이서 --> 앞으로가라 --> 자동차
        * 3). 브레이크
        *   - 사용자 --> 브레이크를 밞아라 --> 카레이서 --> 멈춰라 --> 자동차
        * 4). 시동끄기
        ...
        * */
        ...
        // 카레이서 액셀을 만듦
        CartRaider cartRaider = new CartRaider();
        ...
        while (true){
            ...
            switch (num) {
                case 1:
                    ...
                    break;
                case 2:
                    cartRaider.stepAccelerator();
                    break;
                case 3:
                    cartRaider.stepBreak();
                    break;
                case 4:
                    cartRaider.turnOff();
                    break;
