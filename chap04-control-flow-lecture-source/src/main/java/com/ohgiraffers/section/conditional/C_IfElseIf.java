...
public class C_IfElseIf {
    ...
    *
    * if([조건식1]) {
    *   // 조건식1이 ture 일 떄 실행할 명령문
    * } else if)[조건식2]) {
    *   - 조건1이 false 일때 else if의 조건식2가 ture일떄 실행할 명령문
    * } else {
    *   // 조건식1 false이거. 조건식2가 false일 때 실행할 명령문
    * }
    *
    * if-else-if 문은 조건식1의 결과 값이 참(ture)이면 if {} 안에 있는 코드를 실행하고,
    * 조건식1이 false이면 조건식 2를 확인하여 참(ture)이면 else if {} 안에있는 코드를 실행한다.
    ...
    * */
    ...
    public void testSimpleIfElseIfStatement() {
        ...
        if(answer == 1) {
            ...
            System.out.println("욕심이 과하진 않지만, 넌 거짓말을 하고 있구나!! 어서 썩 사라지거라");
        } else if (answer ==3){
    ...
    public void testNestedIfElseIfStatement(){
         ...
         * */
        System.out.println("ohgiraffers 대학의 김xx교수님은 학생들 시험 성적을 수기로 계산해서 학점 등급을 매기는 채점방식을 사용하고 있었다.");
        System.out.println("90점 이상이면 'A', 80점 이상이면 'B', 70점 이상이면 'C', 60점 이상이면 'D'를");
        System.out.println("60점 미만인 경우에는 'F'를 학점 등급으로하는 기준이다");
        System.out.println("추가로 각 등급의 중간점수(95, 85, 75..)인 경우 '+'를 붙여서 등급을 세분화 하다보니 신경쓸게 이만 저만이 아니다.");
        System.out.println("그래서 학생의 이름과 점수를 입력하면 자동으로 학점 등급이 계산되는 프로그램을 만들기로 했다.");
        System.out.println();
