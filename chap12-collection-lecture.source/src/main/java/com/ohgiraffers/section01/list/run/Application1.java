...
public class Application1 {
    public static void main(String[] args) {
        ...
        // ArrayList 의 크기는 size() 확인할 수 있다.
        // size() 는 사실 요소의 갯수를 반환한다.
        System.out.println("List 의 size : " + list.size());

        // 인덱스가 지정되어있기 때문에 for 문을 접근가능
        for (int i = 0; i < list.size(); i ++){
            System.out.println(i + " : " + list.get(i));
        }

        // arrayList 는 데이터의 중복저장을 허용한다.
        list.add("apple");
        System.out.println("list = " + list);
        // 출력값 = [apple, 123, 45.67, 2024-05-30T14:26:13.781573700, (apple)]
        ...
        * Collections 클래스를 사영 -> Collection 사용되는 기능을 static 메서드로 구현한 클래스*/

        Collections.sort(stringList);
        Collections.reverse(stringList);
        System.out.println("stringList = " + stringList);

        /* ArrayList 에는 기본적인 역순으로 정렬하는 기능이 제공되지 ㅇ낳음
        * LinkedList decendingIterator() 메소드를 사용하면
        * 내림차순으로 정렬된 Iterator 타입의 목록으로 변환
        *
        * Iterator
        * Collection 인터페이스의 iterator() 메소드를 이용햇거 인스턴스를 생성 할 수 있다.
        * 컬렉션에서 값을 읽어오는 방식을 통일되게 제공하기 위해서 사용
        *
        * hasNext() : 다음요소를 가지고 있는 경우 true , 더이상 요소가 없는경우 false 반환
        * next () : 다음요소를 반환 */
